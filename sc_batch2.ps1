...
foreach ($d in $sets) {
  $out = "$outd/noc_sc_vitb_oracle_ours_$($d.n).csv"
  Write-Output "=== START $($d.n)/oracle_ours $(Get-Date -Format 'HH:mm:ss') ==="
  & $py $run --checkpoint $ck --strategy oracle_ours --dataset $d.n `
      --manifest $d.m --data-root $root --out $out
}
Write-Output 'SC_ORACLE_OURS_DONE'
